...
Run the `reset` subcommand from the cluster login node to wipe the remote server in one step (it refuses if a Positron job is currently running):

=== "Alpine"

    ```bash
    ./positron-remote.sh reset alpine
    ```

=== "amc-bodhi"

    ```bash
    ./positron-remote.sh reset bodhi
    ```

Positron will automatically reinstall the correct server version on your next connect.
